...
bool Renderer::initialize(IPlatform* platform) { return true; }
...
bool Renderer::switchBackend(Backend newBackend) { return true; }
...
std::unique_ptr<Renderer> CreateRenderer(Renderer::Backend backend)
{
    ...
    // 当前默认使用 Null 后端
    if (backend == Renderer::Backend::Auto) { backend = Renderer::Backend::Null; }
