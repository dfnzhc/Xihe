...
public:
    ...
    constexpr explicit RandomEngine(uint64_t seed_val) : _engine(seed_val) {}

    RandomEngine() : _engine(GetEntropy()) {}

    constexpr void seed(uint64_t seed_val) { _engine.seed(seed_val); }

    XIHE_NODISCARD constexpr ResultType operator()() { return _engine(); }

    constexpr void setStream(uint64_t stream_id) requires StreamableEngine<E> { _engine.setStream(stream_id); }

    constexpr void advance(int64_t delta) requires JumpableEngine<E> { _engine.advance(delta); }

    XIHE_NODISCARD static constexpr ResultType min() { return E::min(); }
    XIHE_NODISCARD static constexpr ResultType max() { return E::max(); }
...
XIHE_ALWAYS_INLINE RandomEngine<DefaultPRNG>& ThreadLocalRng() { return GetThreadLocalRandomEngine<DefaultPRNG>(); }
